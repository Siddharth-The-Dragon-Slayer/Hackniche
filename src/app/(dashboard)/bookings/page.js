...
const columns = [
    ...
    render: (v) =>
      v ? (
        <span style={{ fontFamily: "var(--font-mono)", fontSize: 12 }}>
          {fmt(v.total_paid)} / {fmt(v.quote_total)}
        </span>
      ) : (
        "—"
      ),
  },
  {
    ...
    label: "Status",
    render: (v) => (
      <Badge variant={STATUS_V[v] || "neutral"}>{STATUS_L[v] || v}</Badge>
    ),
  },
];
...
export default function BookingsPage() {
  ...
  const fetch_ = useCallback(
    async (silent = false) => {
      ...
      try {
        const r = await fetch(
          `/api/bookings?franchise_id=${fid}&branch_id=${bid}`,
        );
  ...
  const tabs = TAB_GROUPS.map((g) => ({
    ...
    count:
      g.key === "all"
        ? bookings.length
        : bookings.filter((b) => g.statuses?.includes(b.status)).length,
  }));
  ...
  const totalRev = bookings.reduce(
    (s, b) => s + (b.payments?.total_paid || 0),
    0,
  );
  const totalBal = bookings.reduce(
    (s, b) => s + (b.payments?.balance_due || 0),
    0,
  );
  ...
  return (
    <motion.div variants={staggerContainer} initial="hidden" animate="visible">
      ...
      >
        <div className="page-header-left">
          ...
          <p style={{ color: "var(--color-text-muted)", fontSize: 14 }}>
            {loading
              ? "Loading…"
              : `${bookings.length} total · ${confirmed} confirmed · ${inProgress} in progress`}
          </p>
        </div>
      ...
      <motion.div variants={fadeUp}>
        <SearchRow
          placeholder="Search by name, phone, hall, event…"
          value={search}
      ...
      <motion.div variants={fadeUp}>
        <DataTable
          ...
          onRowClick={(row) =>
            router.push(
              `/bookings/${row.id}?franchise_id=${fid}&branch_id=${bid}`,
            )
          }
          mobileRender={(row) => (
            <div>
              ...
              >
                ...
                <Badge variant={STATUS_V[row.status] || "neutral"}>
                  {STATUS_L[row.status] || row.status}
                </Badge>
              </div>
              <div
                style={{
              ...
              >
                ...
                <span>
                  <Users size={11} /> {row.expected_guest_count || "?"}
                </span>
                <span style={{ fontWeight: 500 }}>
                  {fmt(row.payments?.total_paid || 0)} /{" "}
                  {fmt(row.payments?.quote_total || 0)}
                </span>
              </div>
            </div>
          )}
