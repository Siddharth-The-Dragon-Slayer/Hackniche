              ...
              {Object.values(row).map((val, j) => (
                <td
                  ...
                  style={{
                    ...
                    fontFamily:
                      typeof val === "number" ? "var(--font-mono)" : "inherit",
                  }}
                >
    ...
    <motion.div variants={staggerContainer} initial="hidden" animate="visible">
      ...
      >
        <div className="page-header-left">
          ...
          <p style={{ color: "var(--color-text-muted)", fontSize: 14 }}>
            Formula: Base Price × SF × DF × OF × ABF
          </p>
        </div>
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <motion.div variants={fadeUp} className="form-card">
            ...
            <div className="form-grid">
              <div className="form-field">
                ...
                <input
                  ...
                  type="number"
                  min="0"
                  value={form.baseHallRent}
                  onChange={(e) => set("baseHallRent", e.target.value)}
                />
              </div>
              <div className="form-field">
                ...
                <input
                  ...
                  type="number"
                  min="0"
                  value={form.basePricePerPlate}
                  onChange={(e) => set("basePricePerPlate", e.target.value)}
                />
              </div>
              <div className="form-field">
            ...
            <div className="form-grid">
              <div className="form-field">
                ...
                />
                <span className="form-hint">
                  Sets Seasonality & Day-of-Week factors
                </span>
              </div>
              <div className="form-field">
                ...
                />
                <span className="form-hint">
                  Default: today — sets Advance Booking factor
                </span>
              </div>
              <div className="form-field form-span-2">
                <label className="form-label">
                  Month Occupancy: <strong>{form.occupancyPercent}%</strong>
                </label>
                <input
                  type="range"
                  min="0"
                  ...
                  onChange={(e) => set("occupancyPercent", e.target.value)}
          ...
          >
            <RefTable
              title="📅 Seasonality Factors (SF)"
              cols={["Season", "Months", "Mult."]}
              rows={SEASONALITY_TABLE.map((r) => ({
                Season: r.season.split(" (")[0],
                Months: r.months,
                mult: r.multiplier,
              }))}
            />
            <RefTable
              title="🗓 Day-of-Week Factors (DF)"
              cols={["Day", "Mult."]}
              rows={DAY_FACTOR_TABLE.map((r) => ({
                Day: r.day,
                mult: r.multiplier,
              }))}
            />
          ...
          <motion.div variants={fadeUp}>
            <RefTable
              title="⏰ Advance Booking Factors (ABF)"
              cols={["Timing", "Type", "Mult."]}
              rows={ADVANCE_BOOKING_TABLE.map((r) => ({
                Range: r.range,
                Type: r.label,
                mult: r.multiplier,
              }))}
          ...
          {!result ? (
            <div className="card" style={{ padding: 36, textAlign: "center" }}>
              ...
              <p style={{ fontSize: 14, color: "var(--color-text-muted)" }}>
                Pick an event date to calculate dynamic price.
              </p>
            </div>
          ) : (
            <>
              ...
              {result.daysAhead > 0 && (
                ...
                >
                  ...
                  >
                    <strong>{result.daysAhead} days</strong> before event ·{" "}
                    {result.abf.label}
                  </span>
                </div>
              )}
