  ...
  const filteredData = inventoryData
    .filter((i) => {
      ...
      if (
        ...
        return false;
      if (statusFilter === "Low Stock" && i.status !== "Low Stock")
        return false;
  ...
  const updateStock = async (itemId, change) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setStats((prev) => {
          const updated = inventoryData.map((i) =>
            i.id === itemId ? { ...i, currentStock: newStock } : i,
          );
          return {
            ...prev,
            lowStockItems: updated.filter(
              (i) =>
                (i.id === itemId ? newStock : i.currentStock) <= i.minStock,
            ).length,
            totalValue: updated.reduce(
              (s, i) =>
                s +
                (i.id === itemId ? newStock : i.currentStock) *
                  (i.pricePerUnit || 0),
              0,
            ),
          };
  ...
  const columns = [
    ...
    { key: "unit", label: "Unit" },
    {
      ...
      render: (v, row) => {
        ...
        const color =
          v <= row.minStock ? "#ef4444" : pct < 40 ? "#f59e0b" : "#10b981";
        return (
    ...
    },
    {
      ...
      label: "Status",
      render: (v) => (
        <Badge variant={v === "Low Stock" ? "red" : "green"}>{v}</Badge>
      ),
    },
    {
      key: "actions",
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          <button
            ...
            style={{
              ...
              fontSize: 11,
              fontWeight: 600,
              opacity: updating === row.id || row.currentStock <= 0 ? 0.4 : 1,
            }}
          >
            -5
          </button>
    ...
    <motion.div variants={staggerContainer} initial="hidden" animate="visible">
      ...
      >
        <div className="page-header-left">
          ...
          <p style={{ color: "var(--color-text-muted)", fontSize: 14 }}>
            {loading
              ? "Loading..."
              : `${stats.totalItems} materials · ${stats.lowStockItems} low stock · ${stats.expiringItems} expiring soon`}
          </p>
        </div>
        <div
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              alignItems: "center",
              gap: 14,
              border: stats.lowStockItems > 0 ? "1px solid #fca5a5" : undefined,
            }}
          >
            <div
              style={{
            ...
            <div>
              <div
                style={{
                  ...
                  fontWeight: 700,
                  color:
                    stats.lowStockItems > 0 ? "#ef4444" : "var(--color-text-h)",
                }}
              >
                {stats.lowStockItems}
              </div>
          ...
          <div
            ...
            style={{
              ...
              alignItems: "center",
              gap: 14,
              border: stats.expiringItems > 0 ? "1px solid #fde68a" : undefined,
            }}
          >
            <div
              style={{
            ...
            <div>
              <div
                style={{
                  ...
                  fontWeight: 700,
                  color:
                    stats.expiringItems > 0 ? "#f59e0b" : "var(--color-text-h)",
                }}
              >
                {stats.expiringItems}
              </div>
      ...
      {showAlerts && (lowStockItems.length > 0 || expiringItems.length > 0) && (
        <motion.div
          variants={fadeUp}
          className="card"
          style={{ padding: 20, marginBottom: 20, border: "1px solid #fca5a5" }}
        >
          <div
            style={{
              display: "flex",
      ...
      <motion.div variants={fadeUp}>
        {loading ? (
          ...
          <DataTable
            ...
            mobileRender={(row) => {
              const pct = row.maxStock
                ? Math.round((row.currentStock / row.maxStock) * 100)
                : 100;
              const barColor =
              ...
              return (
                <div>
                  ...
                  >
                    ...
                    <Badge
                      variant={row.status === "Low Stock" ? "red" : "green"}
                    >
                      {row.status}
                    </Badge>
                  </div>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    <button
                      ...
                      style={{
                        ...
                        fontSize: 12,
                        opacity:
                          updating === row.id || row.currentStock <= 0
                            ? 0.4
                            : 1,
                      }}
                    >
