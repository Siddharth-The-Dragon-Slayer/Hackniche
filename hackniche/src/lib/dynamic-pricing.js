...
export function getOccupancyFactor(occupancyPercent) {
  ...
  const label =
    pct >= 80 ? "High Demand" : pct >= 50 ? "Moderate Demand" : "Low Demand";
  return {
    label: `${label} (${pct}% booked)`,
...
export function getAdvanceBookingFactor(daysBeforeEvent) {
  if (daysBeforeEvent > 120)
    return { label: "Early Bird (>120 days)", multiplier: 0.9 };
  if (daysBeforeEvent >= 60)
    return { label: "Standard (60–120 days)", multiplier: 1.0 };
  if (daysBeforeEvent >= 30)
    return { label: "Short Notice (30–60 days)", multiplier: 1.1 };
...
export function calculateDynamicPrice({
  ...
  const msUntilEvent =
    new Date(eventDate).getTime() - new Date(bookingDate).getTime();
  const daysAhead = Math.max(0, Math.floor(msUntilEvent / 86_400_000));
  const abf = getAdvanceBookingFactor(daysAhead);

  const combinedMultiplier =
    sf.multiplier * df.multiplier * of_.multiplier * abf.multiplier;
  const dynamicPrice = Math.round(basePrice * combinedMultiplier);

  const breakdown = `₹${basePrice.toLocaleString("en-IN")} × ${sf.multiplier} (SF) × ${df.multiplier} (DF) × ${of_.multiplier.toFixed(2)} (OF) × ${abf.multiplier} (ABF)`;
