...
export default function SalesExecutiveDashboard() {
  ...
  const pipelineLeads = shownLeads.filter((l) =>
    PIPELINE_STATUSES.includes(l.status),
  );
  const closedLeads = shownLeads.filter((l) =>
  ...
  const kpis = [
    {
      ...
      value: loading
        ? "…"
        : myLeads.filter((l) => PIPELINE_STATUSES.includes(l.status)).length,
      color: "#6366f1",
    },
    {
      icon: <Clock size={20} />,
      label: "In Pipeline",
      value: loading ? "…" : pipelineLeads.length,
      color: "#f59e0b",
    },
    {
      icon: <CheckCircle2 size={20} />,
      label: "Closed (Won)",
      value: loading ? "…" : closedLeads.length,
      color: "#10b981",
    },
    {
    ...
    <motion.div variants={staggerContainer} initial="hidden" animate="visible">
      ...
      <motion.div variants={fadeUp} style={{ marginBottom: 32 }}>
        <div className="page-header">
          <div className="page-header-left">
            ...
            <p>
              {userProfile?.branch_name || "Branch"} — {name}
            </p>
          </div>
            ...
            return (
              <Link
                ...
                style={{
                  ...
                  border: `2px solid ${count > 0 ? col.color : "var(--color-border)"}`,
                  borderRadius: 12,
                  padding: "14px 16px",
                  textAlign: "center",
              ...
              >
                <div
                  style={{
                    fontSize: 24,
                    fontWeight: 800,
                    color: count > 0 ? col.color : "var(--color-text-muted)",
                  }}
                >
                  {loading ? "…" : count}
                </div>
      ...
      >
        ...
        >
          ...
          >
            Open Pipeline ({pipelineLeads.length})
          </h3>
          <Link
            href="/leads"
            style={{
