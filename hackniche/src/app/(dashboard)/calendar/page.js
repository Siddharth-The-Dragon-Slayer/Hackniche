...
export default function CalendarPage() {
  ...
  const monthBookings = bookings.filter((b) =>
    b.event_date?.startsWith(monthKey),
  );
  const confirmed = monthBookings.filter(
    (b) => b.status === "confirmed",
  ).length;
  const running = monthBookings.filter(
      ...
      >
        <div className="page-header-left">
          ...
          <p style={{ color: "var(--color-text-muted)", fontSize: 14 }}>
            {loading
              ? "Loading…"
              : `${monthBookings.length} events in ${MONTHS[month]} · ${confirmed} confirmed · ${running} running`}
          </p>
        </div>
              ...
              >
                ...
                {events.slice(0, 3).map((ev) => (
                  <div
                    ...
                    onClick={() =>
                      router.push(
                        `/bookings/${ev.id}?franchise_id=${fid}&branch_id=${bid}`,
                      )
                    }
                    style={{
                      fontSize: 10,
                  ...
                  >
                    {ev.event_type ? `${ev.event_type.slice(0, 8)}` : ""}{" "}
                    {ev.customer_name?.split(" ")[0]}
                    {ev.hall_name && (
                      <span style={{ opacity: 0.7 }}> · {ev.hall_name}</span>
                    )}
                  </div>
                ))}
                {events.length > 3 && (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {upcoming.map((ev) => {
                const daysOut = Math.ceil(
                  (new Date(ev.event_date) - new Date(today)) / 86400000,
                );
                return (
                  <div
                    key={ev.id}
                    ...
                    style={{
                      ...
                      alignItems: "center",
                      cursor: "pointer",
                      borderLeft: `3px solid ${daysOut <= 3 ? "#dc2626" : daysOut <= 7 ? "#f59e0b" : "var(--color-primary)"}`,
                    }}
                    onClick={() =>
                      router.push(
                        `/bookings/${ev.id}?franchise_id=${fid}&branch_id=${bid}`,
                  ...
                  >
                    ...
                    <div style={{ textAlign: "right" }}>
                      <Badge variant={STATUS_V[ev.status] || "neutral"}>
                        {STATUS_L[ev.status] || ev.status}
                      </Badge>
                      <div
                        style={{
                          fontSize: 11,
