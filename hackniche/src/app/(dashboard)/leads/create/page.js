...
function numberToIndianWords(num) {
  ...
  let result = "";
  if (num >= 10000000) {
    result += formatBelow1000(Math.floor(num / 10000000)) + " Crore";
    num %= 10000000;
    if (num) result += " ";
  }
  if (num >= 100000) {
    result += formatBelow1000(Math.floor(num / 100000)) + " Lakh";
    num %= 100000;
    if (num) result += " ";
  }
  if (num >= 1000) {
    result += formatBelow1000(Math.floor(num / 1000)) + " Thousand";
    num %= 1000;
    if (num) result += " ";
  }
  result += formatBelow1000(num);
    ...
    try {
      const res = await fetch("/api/leads", {
        ...
        body: JSON.stringify({
          ...
          expected_guest_count: form.expected_guest_count
            ? Number(form.expected_guest_count)
            : null,
      ...
      if (!res.ok) {
        if (res.status === 409) {
          setError(
            `${data.message || data.error}. Existing lead ID: ${data.existing_lead_id}`,
          );
          setSaving(false);
          return;
        }
  ...
  return (
    <div>
        ...
        <div className="page-header-left">
          ...
          <p style={{ color: "var(--color-text-muted)", fontSize: 14 }}>
            {isCustomer
              ...
              : "Capture lead details, assign, and schedule follow-up"}
          </p>
        </div>
      ...
      <div className="form-card">
        ...
        {isCustomer && (
          <>
            <div className="form-section-title" style={{ marginTop: 24 }}>
              <Building2 size={14} /> Preferred Venue
            </div>
            <div className="form-grid">
              <div className="form-field form-span-2">
        ...
        <div className="form-section-title" style={{ marginTop: 24 }}>
          <Calendar size={14} /> Event Requirements
        </div>
        <div className="form-grid">
          <div className="form-field">
          ...
          <div className="form-field">
            ...
            {halls.length === 0 ? (
              <select className="input" disabled>
                <option>
                  {form.branch_id ? "No halls found" : "Select a branch first"}
                </option>
              </select>
            ) : (
              <select
              ...
              >
                ...
                {halls.map((h) => (
                  <option key={h.id} value={h.id}>
                    {h.name} — Cap:{" "}
                    {h.capacity_seating || h.capacity_floating || "?"} |{" "}
                    {fmt(h.base_price)}/day
                  </option>
                ))}
              </select>
            )}
          ...
          <div className="form-field">
            ...
            <select
              className="input"
              value={form.catering_required ? "yes" : "no"}
              onChange={(e) =>
                set("catering_required", e.target.value === "yes")
              }
            >
          ...
          <div className="form-field">
            ...
            <select
              className="input"
              value={form.decor_required ? "yes" : "no"}
              onChange={(e) => set("decor_required", e.target.value === "yes")}
            >
              <option value="yes">Yes</option>
              <option value="no">No — self-arranged</option>
            </select>
        ...
        <div className="form-grid">
          <div className="form-field">
            ...
            {form.budget_min && (
              ...
              >
                {fmt(form.budget_min)} — {numberToIndianWords(form.budget_min)}
              </span>
            )}
          </div>
          <div className="form-field">
            ...
            {form.budget_max && (
              ...
              >
                {fmt(form.budget_max)} — {numberToIndianWords(form.budget_max)}
              </span>
            )}
          </div>
          <div className="form-field">
            ...
            <select
              className="input"
              value={form.budget_flexibility}
              onChange={(e) => set("budget_flexibility", e.target.value)}
            >
              {BUDGET_FLEXIBILITIES.map((f) => (
          ...
          <>
            ...
            <div className="form-grid">
              ...
              <div className="form-field">
                ...
                <input
                  className="input"
                  placeholder="e.g. Google ad campaign name, specific page…"
        ...
        {isStaff && (
          <>
            <div className="form-section-title" style={{ marginTop: 24 }}>
              <UserCheck size={14} /> Assignment &amp; Follow-up
            </div>
            <div className="form-grid">
              <div className="form-field">
              ...
              <div className="form-field">
                ...
                <select
                  className="input"
                  value={form.next_followup_type}
                  onChange={(e) => set("next_followup_type", e.target.value)}
                >
        ...
        {/* ── SECTION 6: Notes ─────────────────────────────────────────── */}
        <div className="form-section-title" style={{ marginTop: 24 }}>
          <MessageSquare size={14} /> {isCustomer ? "Additional Info" : "Notes"}
        </div>
        <div className="form-grid">
          <div className="form-field form-span-2">
            ...
            <textarea
              ...
              placeholder={
                isCustomer
                  ? "Any special requirements, dietary preferences, theme ideas…"
                  : "Any special requirements, preferences, or observations…"
              }
              value={form.notes}
