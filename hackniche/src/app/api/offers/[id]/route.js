...
async function getOfferAndAuthorize(db, id, user) {
  ...
  if (
    ...
    offer.franchise_id !== user.franchise_id
  ) {
    return { error: "Forbidden", status: 403 };
  }
...
export async function PATCH(request, { params }) {
  ...
  const db = getAdminDb();
  const { offer, error, status } = await getOfferAndAuthorize(
    db,
    params.id,
    user,
  );
  ...
  if (isManager) {
    ...
    if (body.discount_value !== undefined)
      update.discount_value = Number(body.discount_value) || 0;
    ...
    if (body.min_guests !== undefined)
      update.min_guests = Number(body.min_guests) || 0;
    ...
    if (body.consumer_visible !== undefined)
      update.consumer_visible = !!body.consumer_visible;
    if (body.single_use !== undefined) update.single_use = !!body.single_use;
    if (body.max_uses !== undefined)
      update.max_uses = Number(body.max_uses) || 0;
  }
...
export async function DELETE(request, { params }) {
  ...
  if (user.role === "sales_executive") {
    return NextResponse.json(
      { error: "Sales executives cannot delete offers" },
      { status: 403 },
    );
  }
