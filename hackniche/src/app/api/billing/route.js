...
export async function GET(req) {
  try {
    ...
    const branch_id = searchParams.get("branch_id") || null; // null = franchise-level
    const status = searchParams.get("status");

    const cacheKey = `invoices:${franchise_id}:${branch_id || "ALL"}:${status || ""}`;
    const cached = cache.get(cacheKey);
    if (cached) return NextResponse.json(cached);

    let snap;
    if (branch_id) {
      // Branch-level: query by branch_id, filter franchise in memory
      let q = adminDb
        .collection("invoices")
        .where("branch_id", "==", branch_id);
      if (status && INV_STATUSES.includes(status))
        q = q.where("status", "==", status);
      snap = await q.limit(500).get();
    } else {
      // Franchise-level: query by franchise_id directly
      let q = adminDb
        .collection("invoices")
        .where("franchise_id", "==", franchise_id);
      if (status && INV_STATUSES.includes(status))
        q = q.where("status", "==", status);
      snap = await q.limit(1000).get();
    }
    ...
    const invoices = snap.docs
      .map((d) => {
        ...
        return {
          id: d.id,
          ...data,
          payments: data.payments || data.payment_history || [],
          balance: data.balance ?? data.balance_due ?? 0,
        };
      })
      .filter((inv) => (branch_id ? inv.franchise_id === franchise_id : true))
      .sort((a, b) => {
...
export async function POST(req) {
  try {
    ...
    if (booking_id) {
      ...
      const foodCost =
        b.menu?.total ||
        (b.menu?.per_plate_cost && b.menu?.plates
          ? b.menu.per_plate_cost * b.menu.plates
          : 0);
      const hallRent = b.payments?.quote_total
        ? b.payments.quote_total - foodCost - (b.decor?.cost || 0)
        : 0;

      const lineItems = [];
      if (foodCost > 0)
        lineItems.push({
          description: `Food — ${b.menu?.name || "Menu"}`,
          quantity: b.menu?.plates || 1,
          rate: b.menu?.per_plate_cost || foodCost,
          amount: foodCost,
        });
      if (hallRent > 0)
        lineItems.push({
      ...
      if (b.decor?.cost > 0)
        lineItems.push({
          description: `Decoration — ${b.decor?.theme || "Standard"}`,
          quantity: 1,
          rate: b.decor.cost,
          amount: b.decor.cost,
        });
      ...
      invoiceData = {
        invoice_number,
        booking_id,
        lead_id: b.lead_id || lead_id || null,
    ...
    return NextResponse.json(
      { message: "Invoice created", invoice_id: ref.id, invoice_number },
      { status: 201 },
    );
  } catch (e) {
