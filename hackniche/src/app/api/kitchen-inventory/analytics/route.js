  ...
  try {
    ...
    if (!franchiseId) {
      return NextResponse.json(
        { success: false, error: "franchise_id is required" },
        { status: 400 },
      );
    }
    ...
    const totalValue = items.reduce(
      (s, i) => s + (i.currentStock || 0) * (i.pricePerUnit || 0),
      0,
    );
    ...
    items.forEach((i) => {
      ...
      if (!categoryMap[cat])
        categoryMap[cat] = { count: 0, value: 0, lowStock: 0 };
      categoryMap[cat].count++;
      categoryMap[cat].value += (i.currentStock || 0) * (i.pricePerUnit || 0);
      if ((i.currentStock || 0) <= (i.minStock || 0))
    ...
    const stockLevels = items
      .map((i) => ({
        ...
        pct: i.minStock
          ? Math.round(((i.currentStock || 0) / i.minStock) * 100)
          : 100,
        unit: i.unit,
    ...
    const monthlySpendChart = Object.entries(monthlySpend).map(
    ...
    orders.forEach((o) => {
      ...
      if (!vendorMap[vn])
        vendorMap[vn] = { name: vn, totalValue: 0, orderCount: 0 };
    ...
    // ── Deduction Stats ──
    const deductionLogs = deductionDoc.exists
      ? deductionDoc.data().logs || []
      : [];
    const totalDeductions = deductionLogs.length;
    const totalGuestsServed = deductionLogs.reduce(
      (s, l) => s + (l.guest_count || 0),
      0,
    );
    const deductionsWithShortages = deductionLogs.filter(
      (l) => l.shortages?.length > 0,
    );
    ...
    deductionLogs.forEach((log) => {
      (log.deductions || []).forEach((d) => {
        if (!materialUsage[d.material])
          materialUsage[d.material] = {
            name: d.material,
            totalUsed: 0,
            unit: d.unit,
          };
    ...
    deductionLogs.forEach((log) => {
      ...
      if (d >= sixMonthsAgo) {
        ...
        if (!monthlyConsumption[key])
          monthlyConsumption[key] = { month: key, guests: 0, deductions: 0 };
    ...
    const analytics = {
      inventory: {
        ...
        expiringSoonItems: expiringSoon.map((i) => ({
          id: i.id,
          name: i.name,
          expiryDate: i.expiryDate,
          daysLeft: Math.ceil((new Date(i.expiryDate) - now) / 86400000),
        })),
        categoryBreakdown,
      ...
      purchaseOrders: {
        ...
        unpaidValue: Math.round(
          unpaidOrders.reduce((s, o) => s + (o.totalAmount || 0), 0),
        ),
