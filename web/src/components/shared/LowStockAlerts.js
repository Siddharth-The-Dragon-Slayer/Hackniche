    ...
    >
      ...
      <div
        ...
        style={{
          ...
          borderBottom: expanded ? "1px solid rgba(239,68,68,0.1)" : "none",
        }}
      >
      ...
      {expanded && (
        <div
          style={{ padding: "12px 16px", maxHeight: 300, overflowY: "auto" }}
        >
          {/* Low Stock */}
          {alerts.lowStock.length > 0 && (
            <div style={{ marginBottom: alerts.expiring.length > 0 ? 12 : 0 }}>
              ...
              {alerts.lowStock.slice(0, 5).map((item) => (
                ...
                >
                  <span
                    style={{ fontWeight: 500, color: "var(--color-text-h)" }}
                  >
                    {item.name}
                  </span>
                  <span
          ...
          {alerts.expiring.length > 0 && (
            <div>
              ...
              {alerts.expiring.slice(0, 5).map((item) => {
                const days = Math.ceil(
                  (new Date(item.expiryDate) - new Date()) / 86400000,
                );
                return (
                  <div
                    key={item.id}
