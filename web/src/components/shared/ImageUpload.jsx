...
export default function ImageUpload({
  ...
  const handleFiles = useCallback(async (files) => {
    ...
    const failed = results
      ...
      .map((r, i) => `${validFiles[i]?.name}: ${r.reason?.message || 'Upload failed'}`);
  ...
  return (
    <div>
      {/* Section label */}
      <div className="form-section-title" style={{ marginTop: 24, marginBottom: 8 }}>
        {label}
      ...
      {images.length > 0 && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 12 }}>
          {images.map((url, idx) => (
            <div
              key={idx}
              style={{ position: 'relative', width: 100, height: 100, borderRadius: 10, overflow: 'hidden', border: '1.5px solid var(--color-border)', flexShrink: 0 }}
            >
          ...
          {Object.entries(progMap).map(([name, pct]) => (
            <div
              key={name}
              style={{ position: 'relative', width: 100, height: 100, borderRadius: 10, border: '1.5px solid var(--color-border)', background: 'var(--color-bg-alt)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6, flexShrink: 0 }}
            >
              <Loader2 size={20} style={{ animation: 'spin 1s linear infinite', color: 'var(--color-primary)' }} />
              <span style={{ fontSize: 10, color: 'var(--color-text-muted)', fontFamily: 'var(--font-mono)' }}>{pct}%</span>
              {/* progress bar */}
              <div style={{ position: 'absolute', bottom: 0, left: 0, height: 3, width: `${pct}%`, background: 'var(--color-primary)', transition: 'width .2s', borderRadius: '0 0 10px 10px' }} />
            </div>
          ))}
        </div>
      )}
      ...
      {canAdd && (
        ...
        >
          {uploading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
              <ImagePlus size={22} style={{ color: 'var(--color-text-muted)', opacity: 0.6 }} />
      ...
      {errors.length > 0 && (
        <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 4 }}>
          {errors.map((err, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 6, fontSize: 12, color: 'var(--color-danger)' }}>
