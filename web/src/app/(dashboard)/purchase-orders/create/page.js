...
function CreatePOInner() {
    ...
    if (prefillParam) {
      try {
        ...
        if (Array.isArray(shortageItems) && shortageItems.length > 0) {
          ...
          setItems(prefilled);
          setPrefillBanner(
            `${shortageItems.length} shortage items pre-filled from stock deduction. Select a vendor and review quantities.`,
          );
        }
      } catch (e) {
  ...
  const setItem = (i, k, v) => {
    const newItems = items.map((it, idx) => {
      if (idx === i) {
        ...
        updated.total =
          (parseFloat(updated.qty) || 0) * (parseFloat(updated.unitPrice) || 0);
        return updated;
      }
      return it;
    });
    ...
    const fetchData = async () => {
      ...
      try {
        const vendorRes = await fetch(
          `/api/kitchen-vendor?franchise_id=${franchiseId}`,
        );
      ...
      try {
        const invRes = await fetch(
          `/api/kitchen-inventory?franchise_id=${franchiseId}`,
        );
        ...
        if (invResult.success) {
          ...
          setItems((prev) =>
            prev.map((it) => {
              ...
              const match = invResult.data.find(
                (inv) =>
  ...
  const vendorItems = vendorCategory
    ? inventoryItems.filter(
        (inv) => inv.category?.toLowerCase() === vendorCategory.toLowerCase(),
      )
    : inventoryItems;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        alert(
          `Failed to create purchase order: ${result.error || "Unknown error"}`,
        );
      }
    } catch (error) {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span style={{ color: "var(--color-text-muted)" }}>
                        {v.contactName && (
                          <>
                            <strong>{v.contactName}</strong> &middot;{" "}
                          </>
                        )}
            ...
            <div style={{ gridColumn: "1 / -1" }}>
              ...
              >
                ...
                <span className="form-label" style={{ marginBottom: 0 }}>
                  Interstate Purchase (IGST applies)
                </span>
              </label>
            </div>
            {!form.isInterstate ? (
              <>
                <div>
                  ...
                  <input
                    ...
                    type="number"
                    step="0.01"
                    value={form.cgstRate * 100}
                    onChange={(e) =>
                      set("cgstRate", parseFloat(e.target.value) / 100 || 0)
                    }
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    type="number"
                    step="0.01"
                    value={form.sgstRate * 100}
                    onChange={(e) =>
                      set("sgstRate", parseFloat(e.target.value) / 100 || 0)
                    }
                  />
                </div>
              </>
              ...
              <div>
                ...
                <input
                  ...
                  type="number"
                  step="0.01"
                  value={form.igstRate * 100}
                  onChange={(e) =>
                    set("igstRate", parseFloat(e.target.value) / 100 || 0)
                  }
                />
              </div>
            )}
        ...
        <div className="card" style={{ padding: 28 }}>
          ...
          {!form.vendorId && (
            ...
            >
              ...
              Select a vendor first — the item list will filter to that
              vendor&apos;s supply category.
            </div>
          )}
          {form.vendorId && vendorItems.length === 0 && (
            ...
            >
              ...
              No inventory items found for category &quot;{form.vendorCategory}
              &quot;. Enter item names manually below.
            </div>
          )}
          {errors.items && (
              ...
              <thead>
                ...
                >
                  <th style={{ padding: "8px 10px", textAlign: "left" }}>
                    Select Item{vendorCategory ? ` (${vendorCategory})` : ""}
                  </th>
                  <th style={{ padding: "8px 10px", textAlign: "left" }}>
                    Item Name
                  </th>
                        ...
                        {(vendorItems.length > 0
                          ...
                          <option key={inv.id} value={inv.id}>
                            ...
                            {inv.currentStock !== undefined
                              ? ` (${inv.currentStock} ${inv.unit} in stock)`
                              : ""}
                          </option>
                        ))}
                      </select>
          ...
          >
            ...
            {!form.isInterstate ? (
              <>
                ...
                >
                  <span style={{ color: "var(--color-text-muted)" }}>
                    CGST ({(form.cgstRate * 100).toFixed(2)}%)
                  </span>
                ...
                >
                  <span style={{ color: "var(--color-text-muted)" }}>
                    SGST ({(form.sgstRate * 100).toFixed(2)}%)
                  </span>
                  <span style={{ fontFamily: "var(--font-mono)" }}>
                    ₹
              ...
              >
                <span style={{ color: "var(--color-text-muted)" }}>
                  IGST ({(form.igstRate * 100).toFixed(2)}%)
                </span>
                <span style={{ fontFamily: "var(--font-mono)" }}>
