...
import {
  Plus, Download, Search, RefreshCw, Package, Truck,
  CheckCircle2, Clock, AlertTriangle, DollarSign, Loader2,
  ArrowUpDown, Filter, PackageCheck,
} from 'lucide-react';
import { useAuth } from '@/contexts/auth-context';

const STATUS_V = { Delivered: 'green', Pending: 'accent', Ordered: 'primary', Cancelled: 'red', Received: 'green' };
const PAYMENT_V = { Paid: 'green', Pending: 'accent', 'Partially Paid': 'accent', Overdue: 'red' };
...
export default function PurchaseOrdersPage() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/purchase-order?franchise_id=${franchiseId}`);
  ...
  const stats = {
    total: data.length,
    pending: data.filter(o => o.status === 'Ordered' || o.status === 'Pending').length,
    delivered: data.filter(o => o.status === 'Delivered' || o.status === 'Received').length,
    totalValue: data.reduce((s, o) => s + (o.totalAmount || 0), 0),
    unpaid: data.filter(o => o.paymentStatus === 'Pending' || o.paymentStatus === 'Overdue').length,
  };
  ...
  const filteredData = data
    .filter(o => {
      if (searchTerm) {
        ...
        if (!o.id?.toLowerCase().includes(term) && !o.vendorName?.toLowerCase().includes(term)) return false;
      }
      if (statusFilter !== 'All' && o.status !== statusFilter) return false;
      if (paymentFilter !== 'All' && o.paymentStatus !== paymentFilter) return false;
      return true;
    })
    .sort((a, b) => new Date(b.created || b.orderDate) - new Date(a.created || a.orderDate));
  ...
  const updateStatus = async (orderId, newStatus) => {
    ...
    try {
      const res = await fetch('/api/purchase-order', {
        ...
        body: JSON.stringify({ franchise_id: franchiseId, id: orderId, status: newStatus }),
      });
      const result = await res.json();
      if (result.success) {
  ...
  const receiveStock = async (order) => {
    if (!confirm(`Receive stock for PO ${order.id}?\nThis will mark the PO as Delivered and add ${order.items?.length || 0} items to inventory.`)) return;
    ...
    try {
      ...
      await fetch('/api/purchase-order', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ franchise_id: franchiseId, id: order.id, status: 'Delivered' }),
      });
      ...
      setData(prev => prev.map(o => o.id === order.id ? { ...o, status: 'Delivered' } : o));
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (v, row) => (
        <div>
          ...
          {row.branchId && <span style={{ fontSize: 11, color: 'var(--color-text-muted)' }}>Branch: {row.branchId}</span>}
        </div>
      )
    },
    {
      ...
      render: v => {
        ...
        return (
          <div style={{ fontSize: 12 }}>
            <span style={{ fontWeight: 600 }}>{v.length} item{v.length !== 1 ? 's' : ''}</span>
            <div style={{ color: 'var(--color-text-muted)', marginTop: 2 }}>
              {v.slice(0, 2).map(i => i.name).join(', ')}
    ...
    },
    {
      ...
      render: v => <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 700, color: 'var(--color-text-h)' }}>{fmt(v)}</span>
    },
    {
      key: 'orderDate', label: 'Ordered',
      render: v => v ? <span style={{ fontSize: 13 }}>{new Date(v).toLocaleDateString('en-GB')}</span> : '—'
    },
    {
      ...
      render: v => {
        ...
        return <span style={{ fontSize: 13, color: isOverdue ? '#ef4444' : undefined, fontWeight: isOverdue ? 600 : 400 }}>{d.toLocaleDateString('en-GB')}</span>;
      }
    },
    {
      ...
      render: (_, row) => (
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          {(row.status === 'Ordered' || row.status === 'Pending') && (
            <button onClick={() => receiveStock(row)} disabled={updating === row.id}
              style={{
                background: '#10b981', color: '#fff', border: 'none', padding: '5px 10px',
          ...
          {row.status === 'Delivered' && (
            <button onClick={() => generateInvoicePDF(row)}
              style={{
                background: '#6366f1', color: '#fff', border: 'none', padding: '5px 10px',
                borderRadius: 6, cursor: 'pointer', fontSize: 11, fontWeight: 600,
                display: 'flex', alignItems: 'center', gap: 4
              }}>
          ...
          {row.status === 'Ordered' && (
            <button onClick={() => updateStatus(row.id, 'Cancelled')} disabled={updating === row.id}
              style={{
  ...
  return (
    <motion.div variants={staggerContainer} initial="hidden" animate="visible">
      ...
      <motion.div variants={fadeUp} className="page-header" style={{ marginBottom: 20 }}>
        <div className="page-header-left">
          ...
          <p style={{ color: 'var(--color-text-muted)', fontSize: 14 }}>
            {loading ? 'Loading...' : `${stats.total} orders · ${stats.pending} pending · ${fmt(stats.totalValue)} total value`}
          </p>
        </div>
        <div className="page-actions" style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          <Link href="/inventory" className="btn btn-outline btn-sm" style={{ textDecoration: 'none' }}>
            <Package size={14} /> Raw Materials
          </Link>
          <Link href="/purchase-orders/create" className="btn btn-primary btn-sm" style={{ textDecoration: 'none' }}>
      ...
      {/* Stats Cards */}
      {!loading && (
        <motion.div variants={fadeUp} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 14, marginBottom: 20 }}>
          <div className="card" style={{ padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <div style={{ fontSize: 20, fontWeight: 700, color: 'var(--color-text-h)' }}>{stats.total}</div>
          ...
          <div className="card" style={{ padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <div style={{ fontSize: 20, fontWeight: 700, color: stats.pending > 0 ? '#f59e0b' : 'var(--color-text-h)' }}>{stats.pending}</div>
              <div style={{ fontSize: 11, color: 'var(--color-text-muted)' }}>Pending Delivery</div>
            </div>
          </div>
          <div className="card" style={{ padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <div style={{ fontSize: 20, fontWeight: 700, color: 'var(--color-text-h)' }}>{stats.delivered}</div>
          ...
          <div className="card" style={{ padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <div style={{ fontSize: 20, fontWeight: 700, color: 'var(--color-text-h)' }}>{fmt(stats.totalValue)}</div>
              <div style={{ fontSize: 11, color: 'var(--color-text-muted)' }}>Total Value</div>
            </div>
          </div>
        </motion.div>
      )}
      ...
      {!loading && stats.unpaid > 0 && (
        <motion.div variants={fadeUp} style={{
          ...
          background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.15)', marginBottom: 20
        }}>
          <AlertTriangle size={18} style={{ color: '#f59e0b', flexShrink: 0 }} />
          <span style={{ fontSize: 14, color: 'var(--color-text-h)' }}>
            <strong>{stats.unpaid} order{stats.unpaid > 1 ? 's' : ''}</strong> have pending/overdue payment
          </span>
        </motion.div>
      )}
      ...
      <motion.div variants={fadeUp} style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        <div style={{ position: 'relative', flex: '1 1 220px', maxWidth: 300 }}>
          ...
          <input className="input" placeholder="Search PO # or vendor..." value={searchTerm}
      ...
      <motion.div variants={fadeUp}>
        {loading ? (
          <div className="card" style={{ padding: 60, textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 14, color: 'var(--color-text-muted)' }}>Loading purchase orders...</div>
          </div>
        ) : (
          <DataTable
            columns={columns}
            ...
            mobileRender={(row) => (
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                  <div>
                    <div style={{ fontWeight: 600, fontSize: 14, color: 'var(--color-text-h)' }}>{row.vendorName}</div>
                    <div style={{ fontSize: 11, color: 'var(--color-text-muted)', fontFamily: 'var(--font-mono)', marginTop: 2 }}>{row.id}</div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <Badge variant={STATUS_V[row.status] || 'neutral'}>{row.status}</Badge>
                    <div style={{ fontFamily: 'var(--font-mono)', fontWeight: 700, marginTop: 4, fontSize: 15 }}>{fmt(row.totalAmount)}</div>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 12, fontSize: 12, color: 'var(--color-text-muted)', marginBottom: 8 }}>
                  <span>{row.items?.length || 0} items</span>
                  <span>Ordered: {row.orderDate ? new Date(row.orderDate).toLocaleDateString('en-GB') : '—'}</span>
                  <span>Expected: {row.expectedDelivery ? new Date(row.expectedDelivery).toLocaleDateString('en-GB') : '—'}</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <Badge variant={PAYMENT_V[row.paymentStatus] || 'neutral'}>{row.paymentStatus}</Badge>
                  {(row.status === 'Ordered' || row.status === 'Pending') && (
                    <button onClick={() => receiveStock(row)} disabled={updating === row.id}
                      style={{ background: '#10b981', color: '#fff', border: 'none', padding: '6px 12px', borderRadius: 6, fontSize: 12, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4, flex: 1, justifyContent: 'center' }}>
                      <PackageCheck size={13} /> Receive Stock
                    </button>
                  )}
                  {row.status === 'Delivered' && (
                    <button onClick={() => generateInvoicePDF(row)}
                      style={{ background: '#6366f1', color: '#fff', border: 'none', padding: '6px 12px', borderRadius: 6, fontSize: 12, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4, flex: 1, justifyContent: 'center' }}>
                      <Download size={13} /> Invoice
                    </button>
                  )}
                </div>
