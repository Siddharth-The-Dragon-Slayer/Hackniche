...
const STATUS_LABELS = {
  ...
  menu_selected: "Menu & Quote",
  advance_paid: "Advance Paid",
  decoration_scheduled: "Decor Finalized",
  paid: "Fully Paid",
  in_progress: "Event Running",
  completed: "Event Completed",
  settlement_complete: "Settlement Done",
  closed: "Closed ✓",
  on_hold: "On Hold",
  lost: "Lost",
};
...
const TAB_GROUPS = [
  ...
  { key: "new", label: "New", statuses: ["new"] },
  {
    key: "active",
    label: "Active",
    statuses: ["visited", "tasting_scheduled", "tasting_done", "menu_selected"],
  },
  {
    key: "paid",
    label: "Booked",
    statuses: ["advance_paid", "decoration_scheduled", "paid"],
  },
  {
    key: "event",
    label: "Event",
    statuses: ["in_progress", "completed", "settlement_complete"],
  },
...
const columns = [
      ...
      <div>
        ...
        {row.lead_source && (
          <div style={{ fontSize: 11, color: "var(--color-text-muted)" }}>
            via {row.lead_source.replace(/_/g, " ")}
          </div>
        )}
      </div>
    ),
  ...
  },
  {
    ...
    label: "Priority",
    render: (v) =>
      v ? <Badge variant={PRIORITY_BADGE[v] || "neutral"}>{v}</Badge> : "—",
  },
  {
    ...
    render: (v) => {
      ...
      return (
        <span
          style={{
            fontSize: 12,
            color: isOverdue ? "#dc2626" : "var(--color-text-body)",
  ...
  },
  {
    ...
    label: "Status",
    render: (v) => (
      <Badge variant={STATUS_VARIANT[v] || "neutral"}>
        {STATUS_LABELS[v] || v}
      </Badge>
    ),
  },
        ...
        <DataTable
          ...
          emptyMessage={
            search
              ...
              : "No leads yet. Create the first one!"
          }
          onRowClick={(row) =>
            router.push(
              `/leads/${row.id}?franchise_id=${franchise_id}&branch_id=${branch_id}`,
            )
          }
          mobileRender={(row) => {
            const isOverdue =
            ...
            return (
              <div>
                ...
                >
                  ...
                  >
                    <Calendar size={11} /> {fmtDate(row.event_date)}
                  </span>
                  <span
                    style={{ display: "flex", alignItems: "center", gap: 3 }}
                  >
                    <Users size={11} /> {row.expected_guest_count || "?"} guests
                  </span>
                  {row.hall_name && (
                    <span
                      style={{ display: "flex", alignItems: "center", gap: 3 }}
                    >
                      <Building2 size={11} /> {row.hall_name}
                    </span>
                  )}
                </div>
                <div
                ...
                >
                  <span>
                    {row.assigned_to_name
                      ? `Assigned: ${row.assigned_to_name}`
                      : "Unassigned"}
                  </span>
                  {row.lead_source && (
                    <span>via {row.lead_source.replace(/_/g, " ")}</span>
                  )}
                  {row.next_followup_date && (
                    <span
                      style={{
                    ...
                    >
                      <Clock size={10} />{" "}
                      {isOverdue ? "Overdue" : fmtDate(row.next_followup_date)}
                    </span>
                  )}
                </div>
              </div>
