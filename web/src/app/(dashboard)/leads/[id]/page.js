...
export default function LeadDetailPage(){
  ...
  useEffect(()=>{
    ...
    getDocs(query(collection(db,'halls'),where('branch_id','==',branch_id)))
      ...
      .catch(()=>{});
    getDocs(query(collection(db,'menus'),where('branch_id','==',branch_id)))
      .then(s=>setMenus(s.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>(a.name||'').localeCompare(b.name||''))))
      .catch(()=>{});
