  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Re-fetch history
        fetch(
          `/api/kitchen-inventory/stock-deduction?franchise_id=${franchiseId}`,
        )
          .then((r) => r.json())
          .then((d) => {
            if (d.success) setHistory(d.data || []);
  ...
  return (
    <motion.div variants={staggerContainer} initial="hidden" animate="visible">
      ...
      >
        ...
        >
          <strong>How it works:</strong> Select menu items and guest count →
          Preview the materials that will be deducted (with 10% wastage buffer)
          →<strong> Edit any quantity</strong> if the auto-calculated amount
          isn&apos;t accurate → Confirm to update inventory.
        </div>
      </motion.div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        ...
        >
          ...
          <div className="card" style={{ padding: 24 }}>
            <div className="form-section-title">
              <Utensils size={14} /> Select Menu Items *
            </div>
            <div
              style={{
                display: "flex",
            ...
            >
              Selected: {selectedDishes.length} item
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <button
              ...
              disabled={
                previewLoading || !guestCount || selectedDishes.length === 0
              }
              style={{ flex: 1 }}
            >
              {previewLoading ? (
        ...
        <motion.div variants={fadeUp}>
          ...
          {preview && (
            <div className="card" style={{ padding: 24 }}>
                  ...
                  {!isEditing ? (
                    ...
                    <button
                      onClick={() => {
                        ...
                        const reset = {};
                        Object.entries(editedDeductions).forEach(
                          ([name, v]) => {
              ...
              >
                <strong>Note:</strong> Quantities include a 10% wastage/spillage
                buffer. Click <strong>Edit Quantities</strong> to adjust any
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                {Object.values(editedDeductions).some(
                  (v) => v.qty !== v.original,
                ) &&
                  !isEditing && (
                    <div
                    ...
                    >
                      ✏️ Some quantities have been manually adjusted. The
                      deduction will use your edited values.
                    </div>
                  )}
              </div>
              ...
              {preview.deductions?.length > 0 && (
                <div style={{ marginBottom: 16 }}>
                  ...
                  {preview.deductions.map((d, i) => {
                    ...
                    const willBeLow =
                      newStock <=
                        (preview.deductions.find(
                          (x) => x.material === d.material,
                        )?.minStock || 0) || d.willBeLowStock;
                    return (
                      <div
                        key={i}
                        style={{
                  ...
                  {preview.shortages.map((s, i) => (
                    ...
                    >
                      ...
                      {s.available !== undefined &&
                        ` (have ${s.available}, short by ${s.deficit})`}
                      {s.reason === "Not found in inventory" &&
                        " — Not in inventory"}
                    </div>
                  ))}
                  <Link
                    href={`/purchase-orders/create?prefill=${encodeURIComponent(
                      JSON.stringify(
            ...
            <div className="card" style={{ padding: 24 }}>
              <div style={{ textAlign: "center", marginBottom: 20 }}>
                ...
                <p style={{ fontSize: 13, color: "var(--color-text-muted)" }}>
                  {result.totalItemsAffected} material
                  {result.totalItemsAffected !== 1 ? "s" : ""} updated for{" "}
                  {result.guest_count} guests
                </p>
                <p
                  style={{
                    fontSize: 12,
                ...
                >
                  ...
                  <span style={{ fontFamily: "var(--font-mono)" }}>
                    ...
                    <span
                      style={{
                        fontWeight: 600,
                        color: d.willBeLowStock ? "#f59e0b" : "#10b981",
                      }}
                    >
                      {d.newStock}
                    </span>
          ...
          {!preview && !result && (
            <div className="card" style={{ padding: 40, textAlign: "center" }}>
              ...
              <p style={{ fontSize: 14, color: "var(--color-text-muted)" }}>
                Select menu items and guest count, then click{" "}
                <strong>Preview Deduction</strong> to see which materials will
                be deducted.
              </p>
            </div>
          )}
            ...
            {history.slice(0, 10).map((log, i) => (
              ...
              >
                <div>
                  ...
                  >
                    {log.guest_count} guests · {log.deductions?.length || 0}{" "}
                    items
                  </span>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  >
                    ...
                    {new Date(log.created).toLocaleTimeString("en-IN", {
                      hour: "2-digit",
                      minute: "2-digit",
                    })}
                  </span>
                  {log.shortages?.length > 0 && (
                    <Badge variant="red">
                      {log.shortages.length} shortage
                      {log.shortages.length > 1 ? "s" : ""}
                    </Badge>
                  )}
                </div>
              </div>
