...
function TrustedBy() {
  ...
  return (
    <section style={{ padding: '36px 0', background: 'var(--color-bg-alt)', overflow: 'hidden' }}>
      <p style={{ textAlign: 'center', fontSize: 11, fontWeight: 700, letterSpacing: 3, textTransform: 'uppercase', color: 'var(--color-text-muted)', marginBottom: 18 }}>
      ...
      <div style={{ display: 'flex', gap: 48, animation: 'ticker 20s linear infinite', whiteSpace: 'nowrap' }}>
        {brands.concat(brands).map((b, i) => (
          <span key={i} style={{ fontSize: 16, fontWeight: 600, color: 'var(--color-text-muted)', opacity: 0.5, fontFamily: 'var(--font-display)', flexShrink: 0 }}>{b}</span>
        ))}
      </div>
    </section>
  );
      ...
      <div className="container">
        ...
        <div style={{ overflowX: 'auto', paddingBottom: 8 }}>
          <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true }}
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-start', gap: 0, minWidth: 'max-content', margin: '0 auto' }}>
            {pipelineSteps.map((s, i) => (
              <motion.div key={i} custom={i} variants={scaleIn} style={{ display: 'flex', alignItems: 'center' }}>
                <div style={{
                  width: 'clamp(64px, 9vw, 80px)', height: 'clamp(64px, 9vw, 80px)',
                  ...
                  <span style={{ fontSize: 'clamp(12px, 2vw, 16px)', marginBottom: 2 }}>{i + 1}</span>
                  <span style={{ textAlign: 'center', lineHeight: 1.2, padding: '0 4px' }}>{s}</span>
                </div>
                {i < 8 && <div style={{ width: 'clamp(16px, 2.5vw, 32px)', height: 2, background: 'var(--gradient-bar)', flexShrink: 0 }} />}
              </motion.div>
            ))}
          </motion.div>
        </div>
...
function CTABanner() {
  return (
    <section style={{ padding: 'clamp(60px, 8vw, 80px) clamp(16px, 5vw, 48px)', background: 'var(--color-primary)', position: 'relative', overflow: 'hidden' }}>
      <div className="texture-grain" style={{ position: 'absolute', inset: 0 }} />
      <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
        style={{ textAlign: 'center', position: 'relative', zIndex: 1, maxWidth: 700, margin: '0 auto' }}>
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.75rem, 4vw, 3rem)', fontWeight: 700, color: '#fff', marginBottom: 16, fontStyle: 'italic' }}>
