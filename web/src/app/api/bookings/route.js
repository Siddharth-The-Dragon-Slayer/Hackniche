...
export async function POST(req) {
  try {
    ...
    if (lead_id) {
      ...
      bookingData = {
        ...
        payments: {
          ...
          balance_due:
            quoteTotal - advPaid - (lead.final_payment?.remaining_amount || 0),
          payment_history: [
            ...(advPaid > 0
              ? [
                  {
      ...
      if (!rest.customer_name || !rest.phone || !rest.event_date) {
        return NextResponse.json(
          { error: "customer_name, phone, event_date required" },
          { status: 400 },
        );
      }
      bookingData = {
        ...
        payments: {
          ...
          balance_due:
            Number(rest.quote_total || 0) - Number(rest.advance_amount || 0),
          payment_history: rest.advance_amount
            ? [
                {
                  date:
                    rest.advance_date || new Date().toISOString().slice(0, 10),
    ...
    return NextResponse.json(
      { message: "Booking created", booking_id: ref.id },
      { status: 201 },
    );
  } catch (e) {
