...
export async function PATCH(request, { params }) {
  ...
  if (!id) return NextResponse.json({ error: "Missing hall id" }, { status: 400 });
  ...
  const update = {
    ...
    ...(body.capacity_seating != null && { capacity_seating: Number(body.capacity_seating) }),
    ...(body.capacity_floating != null && { capacity_floating: Number(body.capacity_floating) }),
    ...(body.base_price != null && { base_price: Number(body.base_price) }),
    ...(body.price_per_plate != null && { price_per_plate: Number(body.price_per_plate) }),
    ...(body.features != null && {
...
export async function DELETE(request, { params }) {
  ...
  if (user.role === "franchise_admin" && hall.franchise_id !== user.franchise_id) {
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  }
