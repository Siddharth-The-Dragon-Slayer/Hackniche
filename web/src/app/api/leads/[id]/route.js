...
import {
  getResend,
  FROM_ADDRESS,
  buildMenuConfirmationEmail,
  buildDecorConfirmationEmail,
  ...
  try {
    ...
    if (!franchise_id || !branch_id) {
      return NextResponse.json(
        { error: "franchise_id and branch_id required" },
        { status: 400 },
      );
    }
    ...
    const follow_ups = followupsSnap.docs.map((d) => {
      ...
      return {
        ...
        done_at: s(fd.done_at),
        created_at: s(fd.created_at),
        next_followup_date: s(fd.next_followup_date),
      };
    });
...
function branchStat(adminDb, batch, branch_id, from, to, now) {
  try {
    ...
    if (from)
      upd[`_stats.leads_by_status.${from}`] =
        admin.firestore.FieldValue.increment(-1);
    if (to)
      upd[`_stats.leads_by_status.${to}`] =
        admin.firestore.FieldValue.increment(1);
  ...
  try {
    ...
    if (action === "log_visit") {
      ...
      batch.update(leadRef, {
        ...
        visited: {
          ...
          visited_by: visited_by || pname || null,
          notes: notes || null,
          rating_from_customer: customer_rating
            ? Number(customer_rating)
            : null,
        },
        status_history: admin.firestore.FieldValue.arrayUnion(
          histEntry("visited", puid, `Visit logged — ${hall_name || ""}`),
        ),
        updated_at: now,
      });
      ...
      return NextResponse.json({
        success: true,
        message: "Property visit logged → Visited",
      });
    }
    ...
    if (action === "schedule_tasting") {
      ...
      batch.update(leadRef, {
        ...
        status_history: admin.firestore.FieldValue.arrayUnion(
          histEntry(
            "tasting_scheduled",
            puid,
            `Tasting scheduled for ${tasting_date}`,
          ),
        ),
        updated_at: now,
      });
      act(
        ...
        puid,
        pname,
        { tasting_date, menu_options_to_present },
      );
      branchStat(
        adminDb,
        batch,
      ...
      return NextResponse.json({
        success: true,
        message: "Food tasting scheduled",
      });
    }
    ...
    if (action === "complete_tasting") {
      ...
      batch.update(leadRef, {
        ...
        status_history: admin.firestore.FieldValue.arrayUnion(
          histEntry(
            "tasting_done",
            puid,
            `Tasting done. Preferred: ${preferred_menu || "TBD"}`,
          ),
        ),
        updated_at: now,
      });
      act(
        ...
        puid,
        pname,
        { preferred_menu, dishes_count: dishes_sampled?.length },
      );
      branchStat(
        adminDb,
        batch,
      ...
      return NextResponse.json({
        success: true,
        message: "Food tasting recorded → Tasting Done",
      });
    }
    ...
    if (action === "finalize_menu") {
      ...
      if (!menu_name || !per_plate_cost) {
        return NextResponse.json(
          { error: "menu_name and per_plate_cost required" },
          { status: 400 },
        );
      }

      const plates = Number(
        expected_plates || current.expected_guest_count || 0,
      );
      ...
      batch.update(leadRef, {
        ...
        menu_finalization: {
          finalized_menu_id: menu_id || null,
          finalized_menu_name: menu_name,
          finalized_date: new Date().toISOString(),
          customizations: customizations || [],
          final_per_plate_cost: Number(per_plate_cost),
          expected_plates: plates,
          total_food_cost: foodTotal,
        },
        quote: {
          hall_base_rent: hallRent,
          food_cost: foodTotal,
        ...
        status_history: admin.firestore.FieldValue.arrayUnion(
          histEntry(
            "menu_selected",
            puid,
            `Menu: ${menu_name}, Total: ₹${totalEstimated.toLocaleString()}`,
          ),
        ),
        updated_at: now,
      });
      act(
        ...
        puid,
        pname,
        { menu_name, per_plate_cost, plates, total_estimated: totalEstimated },
      );
      branchStat(
        adminDb,
        batch,
      ...
      return NextResponse.json({
        success: true,
        message: `Menu finalized. Quote: ₹${totalEstimated.toLocaleString()}`,
      });
    }
    ...
    if (action === "record_advance") {
      ...
      if (!advance_amount || !payment_date) {
        return NextResponse.json(
          { error: "advance_amount and payment_date required" },
          { status: 400 },
        );
      }
      ...
      batch.update(leadRef, {
        ...
        status_history: admin.firestore.FieldValue.arrayUnion(
          histEntry(
            "advance_paid",
            puid,
            `Advance ₹${Number(advance_amount).toLocaleString()} received via ${payment_mode || "cash"}`,
          ),
        ),
        updated_at: now,
      });
      ...
      batch.set(auditRef, {
        ...
        franchise_id,
        branch_id,
        performed_by_uid: puid || null,
        performed_by_name: pname || null,
        details: {
          advance_amount: Number(advance_amount),
          payment_mode,
          transaction_ref,
      ...
      const bookingDoc = {
        ...
        expected_guest_count: Number(
          current.expected_guest_count || eventFin.final_guest_count || 0,
        ),
        menu: {
          name: menuData.finalized_menu_name || null,
          per_plate_cost: Number(menuData.final_per_plate_cost || 0),
          plates: Number(
            menuData.expected_plates || current.expected_guest_count || 0,
          ),
      ...
      if (menuData.total_food_cost) {
        lineItems.push({
          description: `Food – ${menuData.finalized_menu_name || "Menu"}`,
          qty: Number(menuData.expected_plates || 1),
          rate: Number(menuData.final_per_plate_cost || 0),
          amount: Number(menuData.total_food_cost),
        });
      }
      if (quoteData.hall_base_rent) {
        lineItems.push({
          description: `Hall Rent – ${current.hall_name || "Hall"}`,
          qty: 1,
          rate: Number(quoteData.hall_base_rent),
          amount: Number(quoteData.hall_base_rent),
        });
      }
      if (eventFin.decoration_cost) {
        lineItems.push({
          description: `Decoration – ${eventFin.decoration_theme || "Custom"}`,
          qty: 1,
          rate: Number(eventFin.decoration_cost),
          amount: Number(eventFin.decoration_cost),
        });
      }
    ...
    if (action === "finalize_decoration") {
      ...
      const plates = Number(
        final_guest_count || current.expected_guest_count || 0,
      );
      const ppCost = Number(
        final_per_plate || current.menu_finalization?.final_per_plate_cost || 0,
      );
      ...
      batch.update(leadRef, {
        ...
        status_history: admin.firestore.FieldValue.arrayUnion(
          histEntry(
            ...
            puid,
            `Event finalized — ${decor_theme || "TBD theme"}, ${plates} guests`,
          ),
        ),
        updated_at: now,
      });
      act(
        ...
        puid,
        pname,
        { final_guest_count: plates, decor_theme, decor_partner },
      );
      branchStat(
        adminDb,
        batch,
      ...
      return NextResponse.json({
        success: true,
        message: "Decoration & event finalized → Decoration Scheduled",
      });
    }
    ...
    if (action === "record_full_payment") {
      const { remaining_amount, payment_date, payment_mode, transaction_ref } =
        updates;
      if (!remaining_amount || !payment_date) {
        return NextResponse.json(
          { error: "remaining_amount and payment_date required" },
          { status: 400 },
        );
      }
      ...
      batch.update(leadRef, {
        ...
        status_history: admin.firestore.FieldValue.arrayUnion(
          histEntry(
            "paid",
            puid,
            `Full payment ₹${Number(remaining_amount).toLocaleString()} received. Event locked.`,
          ),
        ),
        updated_at: now,
      });
      ...
      return NextResponse.json({
        success: true,
        message: `Full payment ₹${Number(remaining_amount).toLocaleString()} recorded → Paid & Locked`,
      });
    }
    ...
    if (action === "start_event") {
      batch.update(leadRef, {
        ...
        event_execution: {
          ...
          started_by: pname || null,
        },
        status_history: admin.firestore.FieldValue.arrayUnion(
          histEntry("in_progress", puid, "Event started"),
        ),
        updated_at: now,
      });
      act(
      ...
      return NextResponse.json({
        success: true,
        message: "Event started → In Progress",
      });
    }
    ...
    if (action === "complete_event") {
      ...
      batch.update(leadRef, {
        ...
        status_history: admin.firestore.FieldValue.arrayUnion(
          histEntry(
            "completed",
            puid,
            `Event completed. Guests: ${actual_guest_count || 0}`,
          ),
        ),
        updated_at: now,
      });
      ...
      return NextResponse.json({
        success: true,
        message: "Event completed → Completed",
      });
    }
    ...
    if (action === "settle_event") {
      ...
      batch.update(leadRef, {
        ...
        post_event_settlement: {
          ...
          final_guest_count: Number(
            final_guest_count ||
              current.event_execution?.actual_guest_count ||
              0,
          ),
        ...
        status_history: admin.firestore.FieldValue.arrayUnion(
          histEntry(
            "settlement_complete",
            puid,
            `Settled. Final: ₹${(total_final_amount || 0).toLocaleString()}`,
          ),
        ),
        updated_at: now,
      });
      act(
        ...
        puid,
        pname,
        { total_final_amount, amount_paid: totalPaid },
      );
      branchStat(
        adminDb,
        batch,
    ...
    if (action === "close_lead") {
      const {
        ...
        service_rating,
        feedback_text,
        permission_for_testimonial,
        repeat_booking,
      } = updates;

      const quote = current.quote || {};
      const extra = current.post_event_settlement?.extra_charges?.amount || 0;
      const lifetime_value = Number(
        current.post_event_settlement?.total_final_amount ||
      ...
      batch.update(leadRef, {
        ...
        status_history: admin.firestore.FieldValue.arrayUnion(
          histEntry(
            "closed",
            puid,
            `Closed. Rating: ${customer_rating || "N/A"}/5. LTV: ₹${lifetime_value.toLocaleString()}`,
          ),
        ),
        updated_at: now,
      });
      act(
        ...
        puid,
        pname,
        { customer_rating, lifetime_value },
      );
      ...
      return NextResponse.json({
        success: true,
        message: `Lead closed ✅ LTV: ₹${lifetime_value.toLocaleString()}`,
      });
    }
    ...
    if (action === "reactivate") {
      ...
      batch.update(leadRef, {
        ...
        on_hold_until: null,
        status_history: admin.firestore.FieldValue.arrayUnion(
          histEntry(prevStatus, puid, "Reactivated from hold"),
        ),
        updated_at: now,
      });
      act(
        ...
        branch_id,
        "reactivated",
        `Lead reactivated from on_hold → ${prevStatus}`,
        puid,
        pname,
      );
      ...
      return NextResponse.json({
        success: true,
        message: `Lead reactivated → ${prevStatus}`,
      });
    }
    ...
    if (action === "add_followup") {
      ...
      if (!scheduled_date || !followup_type) {
        return NextResponse.json(
          { error: "scheduled_date and followup_type required" },
          { status: 400 },
        );
      }
      ...
      batch.set(fuRef, {
        ...
        call_duration_mins: call_duration_mins
          ? Number(call_duration_mins)
          : null,
        call_answered: call_answered ?? null,
        next_followup_date: nextDate || null,
        next_followup_type: nextType || null,
        done_by_user_id: puid || null,
        done_by_user_name: pname || null,
        done_at: now,
        is_overdue: false,
        created_at: now,
      });
    ...
    if (action === "mark_lost") {
      ...
      batch.update(leadRef, {
        status: "lost",
        lost_reason,
        lost_detail: lost_detail || null,
        competitor_chosen: competitor_chosen || null,
        status_history: admin.firestore.FieldValue.arrayUnion(
          histEntry("lost", puid, `Lost: ${lost_reason}`),
        ),
        updated_at: now,
      });
      act(
        ...
        puid,
        pname,
        { lost_reason, lost_detail, competitor_chosen },
      );
      ...
      batch.set(auditRef2, {
        ...
        performed_by_uid: puid || null,
        performed_by_name: pname || null,
        details: { lost_reason, lost_detail, competitor_chosen },
        created_at: now,
      });
    ...
    if (action === "put_on_hold") {
      ...
      if (!on_hold_reason || !on_hold_until) {
        return NextResponse.json(
          { error: "on_hold_reason and on_hold_until required" },
          { status: 400 },
        );
      }
      ...
      batch.update(leadRef, {
        ...
        status_history: admin.firestore.FieldValue.arrayUnion(
          histEntry(
            "on_hold",
            puid,
            `On hold until ${on_hold_until}: ${on_hold_reason}`,
          ),
        ),
        updated_at: now,
      });
      act(
        ...
        branch_id,
        "status_changed",
        `Lead put on hold until ${on_hold_until} — ${on_hold_reason}`,
        puid,
        pname,
      );
    ...
    if (action === "status_change") {
      ...
      batch.update(leadRef, {
        status: new_status,
        status_history: admin.firestore.FieldValue.arrayUnion(
          histEntry(new_status, puid, note || "Manual status change"),
        ),
        updated_at: now,
      });
      act(
        ...
        branch_id,
        "status_changed",
        `Status: ${current.status} → ${new_status}${note ? " — " + note : ""}`,
        puid,
        pname,
    ...
    act(
      ...
      cleanUpdates.status
        ? `Status changed to ${cleanUpdates.status}`
        : `Lead updated: ${Object.keys(cleanUpdates).join(", ")}`,
      puid,
      pname,
    );
    await batch.commit();
