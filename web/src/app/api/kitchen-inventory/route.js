  ...
  try {
    ...
    const enhancedData = filteredData.map((item) => ({
      ...
      stockPercentage: Math.round(
        (item.currentStock / (item.maxStock || item.currentStock)) * 100,
      ),
    ...
    const stats = {
      ...
      lowStockItems: enhancedData.filter((item) => item.reorderSuggestion)
        .length,
      expiringItems: enhancedData.filter(
        (item) => item.isExpiringSoon && item.daysUntilExpiry > 0,
      ).length,
      totalValue: enhancedData.reduce(
        (sum, item) => sum + (item.totalValue || 0),
        0,
      ),
      categories: [...new Set(enhancedData.map((item) => item.category))]
        .length,
    };
  ...
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Missing required fields: franchise_id, name, category, unit, currentStock, minStock",
        },
        { status: 400 },
      );
    }
    ...
    const newItem = {
      ...
      maxStock: body.maxStock
        ? parseFloat(body.maxStock)
        : parseFloat(currentStock) * 2,
      ...
      status:
        parseFloat(currentStock) <= parseFloat(minStock)
          ? "low-stock"
          : "in-stock",
...
export async function PUT(request) {
  try {
    ...
    const updatedItem = {
      ...
      ...updateData,
      status:
        (updateData.currentStock || items[itemIndex].currentStock) <=
